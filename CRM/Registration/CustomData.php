...
class CRM_Registration_CustomData {
  ...
  public static function cacheCustomGroupSpecs($custom_group_ids) {
    ...
    foreach ($custom_group_ids as $group_id) {
      if (!array_key_exists($group_id, self::$custom_group_spec_cache)) {
        if (is_numeric($group_id)) {
          $group_ids_to_load[] = $group_id;
        } else {
          $group_names_to_load[] = $group_id;
        }
      }
    }

    // load missing groups (by ID)
    if (!empty($group_ids_to_load)) {
      $loaded_groups = civicrm_api3('CustomGroup', 'get', [
          'id'           => ['IN' => $group_ids_to_load],
          'option.limit' => 0,
      ]);
    ...
    // load missing groups (by name)
    if (!empty($group_names_to_load)) {
      $loaded_groups = civicrm_api3('CustomGroup', 'get', [
          'name'         => ['IN' => $group_names_to_load],
          'option.limit' => 0,
      ]);
      foreach ($loaded_groups['values'] as $group) {
        self::$custom_group_spec_cache[$group['id']] = $group;
  ...
  /**
   * Generates the following SQL JOIN statement:
  ...
  /**
   * Generates the following SQL SELECT snippet
   * "`{$group_alias}`.`{$field['column_name']}` AS $field_alias"
   */
  public static function createSQLSelect($group_name, $field_name, $group_alias, $field_alias) {
    $field = self::getCustomField($group_name, $field_name);
    if ($field) {
      return "`{$group_alias}`.`{$field['column_name']}` AS $field_alias";
    } else {
      return '';
    }
  }
